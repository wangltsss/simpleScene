...
const char *fragmentShaderSource = "#version 330 core\n"
    ...
    //"   float attenuation = 1.0/(u_Atten.x + u_Atten.y * u_D + u_Atten.z * u_D * u_D);\n"
    
    "   float diff = max(dot(normalize(Normal), normalize(u_lightPos - Position)), 0.0);\n"
    "   vec4 objColor = texture(myTexture, UV);\n"
    "   FragColor = vec4(ambientColor, 1.0) * objColor + diff * objColor * vec4(diffuseColor, 1.0) * attenuation;\n"
    "}\0";
...
/* ----------------------------------------------------------------------------------------------------------- */
    ...
    unsigned int DistanceID = glGetUniformLocation(shaderProgram, "u_D");
...
/* ----------------------------------------------------------------------------------------------------------- */
    ...
    */
    ...
    float kc = 1.0f, kl = 0.007*1e-4, kq = 0.0002*1e-4;
